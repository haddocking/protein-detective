...
@dag("protein_detective_minimal_workflow")
def minimal_workflow():
    ...
    def filter_af_task(
        ...
        filtered = list(filter_on_density(af_files, query, density_filtered_dir))
        for f in filtered:
    ...
    @task
    def powerfit(options: PowerfitOptions, template: str) -> tuple[str, str]:
        print(f"Running powerfit with options: {options} and template: {template}")
        ...
        if not result_dir.exists():
            with options.target.open("rb") as target_file:
                run(target_file, template, result_dir, options)
    ...
    @task(outlets=[Asset("scores.csv"), Asset("fitted_models.csv")])
    def save_results(scores, fitted_models):
        scores_df = pd.DataFrame(scores)
        scores_df.to_csv("scores.csv", index=False)
        fitted_models_df = pd.DataFrame(fitted_models)
        fitted_models_df.to_csv("fitted_models.csv", index=False)
    ...
    filtered_pdb_files = filter_pdbs_task(pdb_results, pdb_files, pdb_query)
    af_results = af_of_uniprot(uniprot_accessions, limit)
    af_entries = af_fetch_task(af_results)
    filtered_af_files = filter_af_task(af_entries, dquery)
    templates = prep_powerfit(filtered_af_files, filtered_pdb_files)

    solutions = powerfit.partial(options=powerfit_options).expand(template=templates)
    scores = score_report(solutions, top)
    fitted_models = fit_model_task.expand(row=scores)
    save_results(scores, fitted_models)
