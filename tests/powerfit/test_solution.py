...
# Files used from Path(__file__).parent / "fixtures" where generated by
# The solutions.out (pruned to best one) and fit_1.pdb files where generated by
# running the following command:
# ```bash
# powerfit ribosome-KsgA.map 13 KsgA.pdb -a 20 -p 2 -l -d run --num 1 --delimiter ,
# ```
# The input files here where copied from
# <https://github.com/haddocking/powerfit-tutorial>
...
def test_fit_model(ksga_pdb: Path, solution: dict[str, np.ndarray], tmp_path: Path) -> None:
    unfitted_model_file = ksga_pdb
