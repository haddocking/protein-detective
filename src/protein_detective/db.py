...
def save_pdbs(uniprot2pdbs: dict[str, set[PdbResult]], pdb_files: dict[str, Path], con: DuckDBPyConnection):
    con.executemany(
        "INSERT OR IGNORE INTO proteins (uniprot_acc) VALUES (?)", [(uniprot_acc,) for uniprot_acc in uniprot2pdbs]
    )
    pdb_rows = []
...
def save_alphafolds(afs: list[AlphaFoldEntry], con: DuckDBPyConnection):
    ...
    con.executemany(
        "INSERT OR IGNORE INTO proteins (uniprot_acc) VALUES (?)", [(uniprot_acc,) for uniprot_acc in protein_rows]
    )
